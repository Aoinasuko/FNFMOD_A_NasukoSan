...
class ChartingState extends MusicBeatState
{
	...
	**/
	...
	var player2:Character = new Character(0, 0, "a_nasuko");
			...
			for (ii in _song.notes)
			{
				if (ii.startTime <= strum && ii.endTime > strum)
				{
					...
					var note:Note = new Note(strum, originalNote.noteData, originalNote.prevNote, originalNote.isSustainNote, true, originalNote.isAlt,
						originalNote.beat, originalNote.noteType);
	...
	function swapSection(secit:SwagSection)
	{
		...
		for (i in 0...secit.sectionNotes.length)
		{
			...
			var n = [note[0], Std.int(note[1]), note[2], note[3], note[4], note[5]];
			n[1] = (note[1] + 4) % 8;
			newSwaps.push(n);
		}
	...
	function copySection(?sectionNum:Int = 1)
	{
		...
		for (note in _song.notes[daSec - sectionNum].sectionNotes)
		{
			...
			var copiedNote:Array<Dynamic> = [strum, note[1], note[2], note[3], note[5]];
	...
	function updateGrid():Void
	{
		...
		for (section in _song.notes)
		{
			for (i in section.sectionNotes)
			{
				...
				var note:Note = new Note(daStrumTime, daNoteInfo % 4, null, false, true, i[3], i[4], daNoteType);
		...
		for (sec in _song.notes)
		{
			...
			for (i in sec.sectionNotes)
			{
				if (i[0] == note.strumTime && i[1] == note.rawNoteData)
				{
					...
					{
						Debug.logTrace("seleting " + note.strumTime);
						var box = new ChartingBox(note.x, note.y, note);
	...
	private function addNote(?n:Note):Void
	{
		...
		if (n != null)
		{
			Debug.logTrace("not null add/" + n.sustainLength);
		...
		if (n == null)
		{
			var note:Note = new Note(noteStrum, noteData % 4, null, false, true, false, TimingStruct.getBeatFromTime(noteStrum), noteType);
		...
		{
			var note:Note = new Note(n.strumTime, n.noteData % 4, null, n.isSustainNote, true, n.isAlt, TimingStruct.getBeatFromTime(n.strumTime), n.noteType);
			note.beat = TimingStruct.getBeatFromTime(n.strumTime);
			note.rawNoteData = n.noteData;
			note.sustainLength = n.sustainLength;
			note.setGraphicSize(Math.floor(GRID_SIZE), Math.floor(GRID_SIZE));
			note.updateHitbox();
