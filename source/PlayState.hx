...
class PlayState extends MusicBeatState
{
	...
	function songIntro(?dialogueBox:DialogueBox):Void
    {
	...
	function flagcheck():Void 
	{
		if (NoFallflag(curSong) == true) {
			if (health <= 0.00) {
				health = 0.01;
			}
		}
		if (AllPflag(curSong) == true) {
			health -= 5.0;
		}
	}
	...
	function Damageflag(s:String):Bool
	{
		if (s == "antidote") {
			return true;
		}
	...
	function Painflag(s:String):Bool
	{
		if (s == "antidote") {
			if (storyDifficultyText == "Hard") {
				return true;
	...
	function AllPflag(s:String):Bool
	{
		return false;
	}
	...
	function startSong():Void
	{
		...
		var optiony = healthBarBG.y - 170;

		if (NoFallflag(curSong) == true) {
			OptionActive = true;
			SongOptionText_NF = new FlxText(healthBarBG.x + 50, optiony, 600);
			SongOptionText_NF.text = "NoFall : The game won't be over.";
			SongOptionText_NF.size = 26;
			SongOptionText_NF.color = FlxColor.WHITE;
			SongOptionText_NF.setBorderStyle(FlxTextBorderStyle.OUTLINE, FlxColor.BLACK, 2, 1);
			SongOptionText_NF.cameras = [camHUD];
			SongOptionText_NF.alpha = 0;
			FlxTween.tween(SongOptionText_NF, {alpha: 1}, 0.2);
			add(SongOptionText_NF);
			optiony -= 30;
		}

		if (Damageflag(curSong) == true) {
			OptionActive = true;
			SongOptionText_PN = new FlxText(healthBarBG.x - 60, optiony, 840);
			SongOptionText_PN.text = "D.Notes: If you hit the gray notes, take damage.";
			SongOptionText_PN.size = 26;
			SongOptionText_PN.color = FlxColor.WHITE;
			SongOptionText_PN.setBorderStyle(FlxTextBorderStyle.OUTLINE, FlxColor.BLACK, 2, 1);
			SongOptionText_PN.cameras = [camHUD];
			SongOptionText_PN.alpha = 0;
			FlxTween.tween(SongOptionText_PN, {alpha: 1}, 0.2);
			add(SongOptionText_PN);
			optiony -= 30;
		}

		if (Painflag(curSong) == true) {
			OptionActive = true;
			SongOptionText_Pain = new FlxText(healthBarBG.x - 80, optiony, 840);
			SongOptionText_Pain.text = "Pain : If enemy hit a gray note, take small damage.";
			SongOptionText_Pain.size = 26;
			SongOptionText_Pain.color = FlxColor.WHITE;
			SongOptionText_Pain.setBorderStyle(FlxTextBorderStyle.OUTLINE, FlxColor.BLACK, 2, 1);
			SongOptionText_Pain.cameras = [camHUD];
		...
		if (AllPflag(curSong) == true) {
			OptionActive = true;
			SongOptionText_PC = new FlxText(healthBarBG.x + 20, optiony, 700);
			SongOptionText_PC.text = "P.Chal : If you miss the MFC, you lose.";
			SongOptionText_PC.size = 26;
			SongOptionText_PC.color = FlxColor.RED;
			SongOptionText_PC.setBorderStyle(FlxTextBorderStyle.OUTLINE, FlxColor.BLACK, 2, 1);
			SongOptionText_PC.cameras = [camHUD];
			SongOptionText_PC.alpha = 0;
			FlxTween.tween(SongOptionText_PC, {alpha: 1}, 0.2);
			add(SongOptionText_PC);
		}
	...
	public function generateSong(dataPath:String):Void
	{
		...
		for (section in noteData)
		{
			...
			for (songNotes in section.sectionNotes)
			{
				...
				// 確認OK
				var daNoteType:Int = songNotes[5];
				...
				for (susNote in 0...Math.floor(susLength))
				{
					...
					var sustainNote:Note = new Note(daStrumTime + (Conductor.stepCrochet * susNote) + Conductor.stepCrochet, daNoteData, oldNote, true, false, false, 0, daNoteType);
	...
	override public function update(elapsed:Float)
	{
		...
		if (OptionActive && Conductor.songPosition >= 5000) {
			OptionActive = false;
			remove(SongOptionText_NF);
	...
	private function keyShit():Void // I've invested in emma stocks
	{
		...
		if ((KeyBinds.gamepad && !FlxG.keys.justPressed.ANY))
		{
			...
			if (pressArray.contains(true) && generatedMusic)
			{
				...
				if (perfectMode && possibleNotes[0].noteType != 1)
				...
				if (boyfriend.holdTimer > Conductor.stepCrochet * 4 * 0.001 && (!holdArray.contains(true) || (PlayStateChangeables.botPlay && possibleNotes[0].noteType != 1)))
				{
